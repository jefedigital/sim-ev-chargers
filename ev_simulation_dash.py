    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Div([
             ...
             # Data Table Row
            dash_table(drivers-table, style={'margin-top': '20px'})
...
def update_output(n_clicks_run, n_clicks_optimize, route, num_drivers, num_stations, num_chargers):
    ...
    if button_id == 'optimize-button':
        optimal_stations = parameter_sweep_chargers(params)
        if optimal_stations > 0:
            # log_msg(f"Num Stations {num_stations} -> Optimal Stations {optimal_stations}")
            params.num_stations = optimal_stations
            results_df, results_dict = run_simulation(params)
            stations_slider_value = optimal_stations
        else:
            optimal_stations = None
            stations_slider_value = num_stations
    ...
    drivers_table = dash_table.DataTable(results_df.to_dict('records'))
    ...
    return (
        ...
        # map_fig,
        drivers_table,
        stations_slider_value, # slider value
        f"{optimal_stations}", # text box value
    )
